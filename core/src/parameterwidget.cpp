...
void ParameterWidget::saveButtonClicked()
{
    ...
    for (int i = 0; i < m_fieldConfigList.size(); i++) {
        ...
        if (editsplit.size() == 0) {
            ...
            QLOG_DEBUG() << "Saving:"
                         << "0x" + QString::number(m_fieldConfigList[i].second.locationId(), 16).toUpper() << "Offset:" << QString::number(m_fieldConfigList[i].second.offset()) << valstr;
...
void ParameterWidget::updateValue(unsigned short locationid, QByteArray block)
{
    for (int i = 0; i < m_fieldConfigList.size(); i++) {
        if (m_fieldConfigList[i].second.locationId() == locationid) {
            ...
            QLOG_DEBUG() << "Pre value:"
                         << "0x" + QString::number(locationid, 16) << bytestr;
