...
#include <QJsonObject>
#include <QJsonArray>
FreeEmsComms::FreeEmsComms(QObject *parent)
    : EmsComms(parent)
{
...
bool FreeEmsComms::sendPacket(RequestClass request)
{
    ...
    QLOG_DEBUG() << "sendPacket:"
                 << "0x" + QString::number(request.type, 16).toUpper();
    if (!request.hasReply) {
    ...
    if (!m_waitingForResponse) {
        ...
        QLOG_DEBUG() << "sendPacket packet queued:"
                     << "0x" + QString::number(request.type, 16).toUpper();
        m_reqList.append(request);
    }
    return false;
...
bool FreeEmsComms::sendPacket(unsigned short payloadid, QList<QVariant> arglist, QList<int> argsizelist, bool haslength)
{
    ...
    QLOG_TRACE() << "Sent packet"
                 << "0x" + QString::number(payloadid, 16).toUpper() << payload.size();
...
void FreeEmsComms::packetNakedRec(unsigned short payloadid, QByteArray header, QByteArray payload, unsigned short errornum)
{
    QLOG_DEBUG() << "Packet nak"
                 << "0x" + QString::number(payloadid, 16).toUpper();
    ...
    if (m_waitingForResponse) {
        ...
        if (payloadid == m_payloadWaitingForResponse + 1) {
            QLOG_TRACE() << "Recieved Negative Response"
                         << "0x" + QString::number(m_payloadWaitingForResponse + 1, 16).toUpper() << "For Payload:"
                         << "0x" + QString::number(m_payloadWaitingForResponse + 1, 16).toUpper() << "Sequence Number:" << m_currentWaitingRequest.sequencenumber;
            ...
            QLOG_ERROR() << "ERROR! Invalid packet:"
                         << "0x" + QString::number(payloadid, 16).toUpper() << "Waiting for"
                         << "0x" + QString::number(m_payloadWaitingForResponse + 1, 16).toUpper();
            QLOG_DEBUG() << payload.toHex();
        }
    }
    return;
    if (m_reqList.size() > 0) {
        QLOG_DEBUG() << "Sending next packet:"
                     << "0x" + QString::number(m_reqList.at(0).type, 16) << "Packets left:" << m_reqList.size();
...
void FreeEmsComms::packetAckedRec(unsigned short payloadid, QByteArray header, QByteArray payload)
{
    ...
    if (m_waitingForResponse) {
        ...
        if (payloadid == m_payloadWaitingForResponse + 1) {
            ...
            m_waitingForResponse = false;
            QLOG_DEBUG() << "Resending packet:"
                         << "0x" + QString::number(m_currentWaitingRequest.type, 16) << "Packets left:" << m_reqList.size();
